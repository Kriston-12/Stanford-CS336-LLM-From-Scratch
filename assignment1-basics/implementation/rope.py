...
class RoPE(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # max_seq_len might not be the same as x's seq_len.
        # considering in real applications, requests have different seqeucens (seq_lens)
        seq_len = x.shape[-2] # by convention, the last two dims are (seq_len, d) 
